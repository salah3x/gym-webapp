...
import { ClientWithId, Client } from '../../shared/client.model';
...
export class ClientsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.searchTerm
      .pipe(
        tap(s =>
          s.trim() ? (this.isLoading = true) : (this.isLoading = false)
        ),
        debounceTime(1000),
        switchMap(s => {
          if (!s.trim()) {
            ...
            return combineLatest(
              this.afs
                .collection<Client>('clients', ref =>
                  ref.orderBy('name.first_lowercase')
                )
                .snapshotChanges(),
              of([]),
              of([])
          ...
          if (s === ':f') {
            return combineLatest(
              this.afs
                .collection<Client>('clients', ref =>
                  ref.where('sex', '==', 'f').orderBy('name.first_lowercase')
                )
                .snapshotChanges(),
              of([]),
              of([])
          ...
          if (s === ':m') {
            return combineLatest(
              this.afs
                .collection<Client>('clients', ref =>
                  ref.where('sex', '==', 'm').orderBy('name.first_lowercase')
                )
                .snapshotChanges(),
              of([]),
              of([])
          ...
          const firstNameRef = this.afs.collection<Client>('clients', ref =>
            ref
              .orderBy('name.first_lowercase')
              .startAt(s)
              .endAt(s + '\uf8ff')
          );
          const lastNameRef = this.afs.collection<Client>('clients', ref =>
            ref
              .orderBy('name.last_lowercase')
              .startAt(s)
              .endAt(s + '\uf8ff')
          );
          const cinRef = this.afs.collection<Client>('clients', ref =>
            ref
              .orderBy('cin')
              .startAt(s)
              .endAt(s + '\uf8ff')
        ...
        map(
          array =>
            Array.from(new Set(array.map(c => c.id))).map(id =>
              array.find(c => c.id === id)
            )
