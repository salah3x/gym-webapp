...
async function isSuperAdminMiddleware(
  req: express.Request,
  ...
  async (req: express.Request, res: express.Response) => {
    ...
    if (!email || !claims) {
      res
        ...
        .send({
          error: 'Bad Request',
          message: 'Email and claims must not be null'
        });
      return;
    }
    if (!claims.superadmin && email === 'salah.loukili@gmail.com') {
      res
        .status(400)
        .send({ error: 'Bad Request', message: 'Cannot do that !' });
      return;
    }
    ...
    try {
      ...
      res
        ...
        .send({
          ...
          message: 'Retrieving user by email failed'
        });
      return;
    }
    return admin
      ...
      .then(() => {
        res
          .status(202)
          .send({ message: `Claims have been set to ${email} successfully` });
      })
      .catch(() => {
        res
          .status(500)
          .send({
            error: 'Internal Error',
            message: `Setting claims to ${email} failed`
          });
      });
  }
);
route.post('/users', (req: express.Request, res: express.Response) => {
  return admin
    ...
    .then(users =>
      res.json(
        users.users
          .filter(u => u.email !== 'salah.loukili@gmail.com')
          .map(user => {
            return {
              email: user.email,
    ...
    .catch(() =>
      res
        .status(500)
        .send({ error: 'Internal Error', message: `Retrieving users failed` })
    );
});
